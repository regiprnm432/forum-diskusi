...
const DropdownDiscussion = ({ user_id, showVerifiy, commentId, setIsVerified, isVerified, path, id}: DropdownDiscussionProps) => {
        ...
        {(isAdmin && showVerifiy) && (
          <Button variant="ghost" onClick={handleVerify}>
            <DropdownMenuItem className="font-light text-lg">
              ...
              <span className="ml-2">{isVerified ? "Batal Verifikasi" : "Verifikasi"}</span>
            </DropdownMenuItem>
          </Button>
        )}
        {user_id === username && (
